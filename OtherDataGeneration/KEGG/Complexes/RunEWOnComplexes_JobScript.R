## This script was used to generate AllPairs data for Complexes on distributed compute
...
load("ProteinComplexTrees.RData") # 30MB, ComplexTrees
load("ComplexesWithPositions.RData") # 15MB, inds
load('KEGGSpeciesTree.RData') # tree -
load("ComplexSpeciesTree.RData") # ComplexSpecTree -

end_row <- ifelse(end_row > nrow(Pairings), nrow(Pairings), end_row)

## normalize species tree heights
h1 <- attr(ComplexSpecTree, 'height')
h2 <- attr(tree, 'height')

SpecTree <- dendrapply(ComplexSpecTree, \(x){attr(x, "height") <- attr(x,'height')/h1;x})
...
kos_used <- unique(unlist(Subpairings))
coloc_modules <- inds[kos_used]
subpw <- ComplexTrees[kos_used]
...
set.seed(635L)
p1 <- predict(pw_nocoloc_orig, Method=algs_nocoloc, Verbose=TRUE, Subset=Subpairings, ReturnDataFrame=FALSE)
...
for(i in seq_along(algs_coloc)){
	...
	for(j in seq_len(nrow(Subpairings))){
		vs <- unlist(Subpairings[j,])
		results_orig[j,i+offset] <- preds_orig[vs[1], vs[2]]
		results_kegg[j,i+offset] <- preds_kegg[vs[1], vs[2]]
	}
}
...
save(RETURNDATA, file=paste0("ComplexResult_", subval-1L, ".RData"))
