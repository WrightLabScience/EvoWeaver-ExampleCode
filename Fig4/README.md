...
then using `min(lab_tree1, lab_tree2)` to get the minimum of the number of leaves between each tree. This computation is straightforward
but time consuming, so the results are provided precomputed within `StringPredictions50.RData`.
