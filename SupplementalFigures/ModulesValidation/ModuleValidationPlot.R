outfile <- 'SXX_ModulesVerification.pdf'
infiles <- c(file.path(datadir, "Modules", "ExtendedModuleEnsembleMethods.RData"),
              file.path(outdir, "ModuleStatistics_modulegeneholdouts.RData"),
              file.path(outdir, "ModuleStatistics_fullmoduleholdouts.RData"))
titles <- c("Modules Benchmark with Extended Ensemble Models",
            "Modules Benchmark with Gene Holdouts",
            "Modules Benchmark with Module Holdouts")
...
for(i in seq_along(infiles)){
  load(infiles[i])
  to_plot <- EnsembleBlockStatistics[alg_names]
...
for(i in seq_along(all_aurocs)){
  ...
  legend('topleft',
         ...
         inset=c(X_OFF +0.37+i*MULT, Y_OFF), xpd=NA,
         cex=LEGEND_CEX
         )
}
...
## add this as a multi column table later
#ROCs <- vapply(to_plot, \(x) x$AUROC, numeric(1L))
