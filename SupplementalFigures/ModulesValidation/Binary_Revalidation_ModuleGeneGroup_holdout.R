## This script will redo binary classification using gene group holdouts
...
all_blocks <- unique(unlist(subPairings))
...
blocksamp <- sample(seq_len(NUM_CV_FOLD), length(all_blocks), replace=TRUE)
names(blocksamp) <- all_blocks
anscol <- which(colnames(testingResults) == 'isTP')

groupings <- cbind(blocksamp[subPairings[[1]]],
                    blocksamp[subPairings[[2]]])
...
save(BlockStatistics, EnsembleBlockStatistics, RawScores,
     file=file.path(outdir, 'ModuleStatistics_modulegeneholdouts.RData'))
